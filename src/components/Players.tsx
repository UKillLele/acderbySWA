...
const Players = () => {
  ...
  const [updatingPositions, setUpdatingPositions] = useState<
    { id: string; positionType: number }[]
  >([]);
  const [tempUpdatingPosition, setTempUpdatingPosition] = useState<{
    id: string;
    positionType: number;
  }>({ id: "", positionType: 0 });
  ...
  function onAddUpdatingPosition() {
    if (
      tempUpdatingPosition.id &&
      !updatingPositions.some((x) => x.id === tempUpdatingPosition.id)
    ) {
  ...
  function onTeamsSelect(event: ChangeEvent) {
    ...
    if (!positions.some((x) => x.id === selectedteamId)) {
      setTempPosition({
        id: selectedteamId,
        positionType: tempPosition.positionType,
      });
    }
  }
  ...
  function onUpdatingTeamsSelect(event: ChangeEvent) {
    ...
    if (!updatingPositions.some((x) => x.id === selectedteamId)) {
      setTempUpdatingPosition({
        id: selectedteamId,
        positionType: tempUpdatingPosition.positionType,
      });
    }
  }
  ...
  function onPlayerUpdate(event: FormEvent) {
    ...
    if (tempUpdatingPosition.id.length > 0)
      setPositions([...updatingPositions, tempUpdatingPosition]);
    setTempUpdatingPosition({ id: "", positionType: 0 });
              ...
              <Container fluid className="mt-0 border bg-dark rounded">
                ...
                {positions &&
                  positions.map((position, index) => (
                    <Row
                      className="p-2"
                      key={`${position.id}-${position.positionType}-${index}`}
                    >
                      <Col>
                        ...
                        <p>
                          {teams.find((x) => x.id === position.id)?.name} -{" "}
                          {GetPositionDisplayName(position.positionType)}
                        </p>
                      </Col>
                      <Col xs="auto">
                        <Button
              ...
              updatingId !== skater.id ? (
                ...
                >
                  ...
                  <Container className="mt-0 border bg-dark rounded">
                    <Row>
                      <p
                        className={`${skater.number && "d-flex justify-content-between px-3"} text-center fs-3 m-0`}
                      >
                    ...
                    {skater.teams &&
                      skater.teams.map((t: Position) => (
                        <Row key={`${skater.id}-${t.id}`}>
                          <p>
                            {getTeamName(t.id)} -{" "}
                            {GetPositionDisplayName(t.positionType)}
                          </p>
                        </Row>
                      ))}
                  </Container>
                ...
                >
                  <Form onSubmit={onPlayerUpdate}>
                    ...
                    <Container fluid className="mt-0 border bg-dark rounded">
                        ...
                        <Col xs lg="4">
                          <Form.Group controlId="updateNumber">
                            <Form.Control
                              ...
                              onChange={(event) =>
                                setUpdatingNumber(event.target.value)
                              }
                              type="string"
                              pattern="[0-9]*"
                              placeholder="#"
                      ...
                      {updatingPositions &&
                        updatingPositions.map((position, index) => (
                          <Row
                            className="p-2"
                            key={`${position.id}-${position.positionType}-${index}`}
                          >
                            ...
                            <Col xs="auto">
                              <Button
                                ...
                                variant="danger"
                                onClick={() =>
                                  onDeleteUpdatingPosition(position.id)
                                }
                              >
                                &times;
                              </Button>
                      ...
                      <Row className="p-2">
                        ...
                        <Col xs="auto">
                          <Button type="button" onClick={onAddUpdatingPosition}>
                            +
                          </Button>
                        </Col>
                      </Row>
